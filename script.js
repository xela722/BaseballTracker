...
function updateLineTxt(team, scores, r, h, e) {
  ...
  txt += "</td><td>" + r + "</td><td>" + h + "</td><td>" + e + "</td></tr>";
}
          ...
          if (x[i].childNodes[j].attributes["pos"].value !== "P") {
            if (flag == "away") {
              awayRos.push(
                new playerObject(
                  flag,
                  x[i].childNodes[j].attributes["id"].value,
                  x[i].childNodes[j].attributes["name"].value,
                  x[i].childNodes[j].attributes["pos"].value
                )
              );
            } else {
              homeRos.push(
...
function createTables(type) {
  if (type == "roster") {
    ...
    tbl +=
      "<td>R</td><td>H</td><td>E</td></tr></thead><tbody id='Line'></tbody></table>";
...
function fillTables(xml) {
  document.getElementById("rosters").innerHTML =
    "<table class='awayRoster'><thead><tr><td>MLB ID</td><td>Name</td><td>Position</td></tr></thead><tbody id='away-Roster'></tbody></table><table class='homeRoster'><thead><tr><td>MLB ID</td><td>Name</td><td>Position</td></tr></thead><tbody id='home-Roster'></tbody></table>";
  teamParse("away", xml);
  teamParse("home", xml);
      ...
      for (var i = 0; i < game.length; i++) {
        games.push(
          new gameObject(
            ...
            game[i].attributes["time_date"].value.substr(
              10,
              game[i].attributes["time_date"].value.length
            ),
